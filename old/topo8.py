...
def frame_to_figure(frame: Dict) -> go.Figure:
    ...
    for e in frame["edges"]:
        # two endpoints + None sentinel => 3 x‑coords per edge
        edge_x.extend([e["x0"], e["x1"], None])
        edge_y.extend([e["y0"], e["y1"], None])
        widths.extend([e["w"]] * 3)  # replicate so len matches coords

    # map weight 0.1–3.0 → stroke px 0.5–4.0
    w_px = [0.5 + 1.5 * (w - 0.1) / 2.9 for w in widths]

    edge_trace = go.Scatter(
        x=edge_x,
        y=edge_y,
        mode="lines",
        line=dict(width=w_px, color="#9e9e9e"),
        hoverinfo="skip",
    )
