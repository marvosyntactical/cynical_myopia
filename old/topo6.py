"""
Iterated PD Echo‑Chamber Simulator — Dash web‑app (v2.1)
=======================================================
Blue = optimists (cooperate); orange = cynics (defect).

*   Edge length & stroke ≈ trust weight ω (strong co‑op tie ⇒ short & thick).
*   GIF exporter (needs **kaleido pillow imageio**).
*   One‑click static build via `python freeze.py`.

Install & run
-------------
```bash
pip install dash==2.17.0 dash-bootstrap-components plotly networkx numpy pillow imageio kaleido
...
def run_sim(prm: Params) -> List[Dict]:
    ...
    types = np.array([
        "optimist" * int(prm.n * prm.pct_opt) + "cynic" * (prm.n - int(prm.n * prm.pct_opt))
    ]).flatten()
    types[: int(prm.n * prm.pct_opt)] = "optimist"
    types[int(prm.n * prm.pct_opt) :] = "cynic"
        ...
        nodes = [
            {
                "x": float(pos[v][0]),
                "y": float(pos[v][1]),
                "c": "#1f77b4" if types[v] == "optimist" else "#ff7f0e",
            }
            for v in G.nodes()
        ]
        edges = [
...
controls = dbc.Card([
    ...
    dbc.Button("Download GIF", id="btn-dl", color="secondary", className="mt-1"),
], body=True)
...
def frame_to_figure(frame: Dict) -> go.Figure:
    ...
    edge_trace = go.Scatter(
        ...
        y=edge_y,
        mode="lines",
        line=dict(width=[0.2 + 1.8 * (wi - 0.1) / 2.9 for wi in w], color="#999"),
        hoverinfo="skip",
    )
    node_trace = go.Scatter(
        ...
        mode="markers",
        marker=dict(color=[n["c"] for n in frame["nodes"]], size=10),
        hoverinfo="skip",
    )
...
def run_callback(n_clicks,
    ...
    prm = Params(
        n=int(n), pct_opt=pct, p_edge=p_edge,
        steps=int(steps), frame_delta=int(frame_delta),
        rew_phi=phi, imitate_prob=imit, tremble_p=trem,
    )
    frames = run_sim(prm)
...
def slider_callback(idx, frames):
    if not frames:
...
# --- download GIF -----------------------------------------------------------
@app.callback(
    Output("gif-dl", "data", allow_duplicate=True),
    Input("btn-dl", "n_clicks"),
    State("frames", "data"),
    prevent_initial_call=True,
)
def gif_callback(n_clicks, frames):
    if not n_clicks or not frames:
        raise dash.exceptions.PreventUpdate

    # render each frame -> PNG bytes with kaleido
